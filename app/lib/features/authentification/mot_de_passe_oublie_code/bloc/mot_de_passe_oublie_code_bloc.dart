...
class MotDePasseOublieCodeBloc extends Bloc<MotDePasseOublieCodeEvent, MotDePasseOublieCodeState> {
  MotDePasseOublieCodeBloc({required final AuthentificationRepository authentificationRepository, required final String email})
    : super(MotDePasseOublieCodeState.initialize(email: email)) {
    on<MotDePasseOublieCodeCodeChange>((final event, final emit) => emit(state.copyWith(code: event.valeur)));
    on<MotDePasseOublieCodeRenvoyerCodeDemande>((final event, final emit) async {
