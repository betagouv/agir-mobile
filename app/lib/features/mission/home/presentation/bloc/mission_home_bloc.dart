...
class MissionHomeBloc extends Bloc<MissionHomeEvent, MissionHomeState> {
  MissionHomeBloc({required final MissionHomeRepository repository}) : super(const MissionHomeInitial()) {
    on<MissionHomeFetch>((final event, final emit) async {
      final result = await repository.fetch();
      result.fold((final l) => emit(const MissionHomeInitial()), (final r) => emit(MissionHomeLoadSuccess(r)));
    });
  }
}
