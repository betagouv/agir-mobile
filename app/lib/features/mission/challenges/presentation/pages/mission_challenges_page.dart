...
class MissionChallengesPage extends StatelessWidget {
  ...
  Widget build(final context) => BlocProvider(
    create: (final context) => MissionChallengesBloc(repository: context.read())..add(MissionChallengesRefreshRequested(code)),
...
class _View extends StatelessWidget {
  ...
  @override
  Widget build(final context) => BlocBuilder<MissionChallengesBloc, MissionChallengesState>(
    builder:
            ...
            ...state.challenges.values
                .map(
                  (final e) => ChallengeWidget(
                    challenge: e,
                    onChanged: () {
                      context.read<MissionChallengesBloc>().add(MissionChallengesRefreshRequested(code));
                    },
                  ),
                )
                  ...
                  child: DsfrButton(
                    ...
                    onPressed:
                        state.challenges.canBeCompleted
                            ? () => context.read<MissionBloc>().add(const MissionCompleteRequested())
                            : null,
                  ),
                ),
              ),
