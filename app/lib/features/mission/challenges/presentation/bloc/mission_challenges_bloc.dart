...
class MissionChallengesBloc extends Bloc<MissionChallengesEvent, MissionChallengesState> {
  MissionChallengesBloc({required final MissionChallengesRepository repository})
    : super(const MissionChallengesState(challenges: MissionChallenges(values: [], canBeCompleted: false, isCompleted: false))) {
    on<MissionChallengesRefreshRequested>((final event, final emit) async {
      final result = await repository.fetch(event.code);

      result.fold(
        (final l) => emit(
          const MissionChallengesState(challenges: MissionChallenges(values: [], canBeCompleted: false, isCompleted: false)),
        ),
