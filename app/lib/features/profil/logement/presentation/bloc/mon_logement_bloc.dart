...
class MonLogementBloc extends Bloc<MonLogementEvent, MonLogementState> {
  MonLogementBloc({required final ProfilRepository profilRepository, required final CommunesRepository communesRepository})
  ...
  Future<void> _onRecuperationDemandee(final MonLogementRecuperationDemandee event, final Emitter<MonLogementState> emit) async {
  ...
  void _onCommuneChange(final MonLogementCommuneChange event, final Emitter<MonLogementState> emit) =>
      emit(state.copyWith(commune: event.valeur));

  void _onNombreAdultesChange(final MonLogementNombreAdultesChange event, final Emitter<MonLogementState> emit) =>
      emit(state.copyWith(nombreAdultes: event.valeur));

  void _onNombreEnfantsChange(final MonLogementNombreEnfantsChange event, final Emitter<MonLogementState> emit) =>
      emit(state.copyWith(nombreEnfants: event.valeur));

  void _onNombreTypeDeLogementChange(final MonLogementTypeDeLogementChange event, final Emitter<MonLogementState> emit) =>
      emit(state.copyWith(typeDeLogement: event.valeur));

  void _onEstProprietaireChange(final MonLogementEstProprietaireChange event, final Emitter<MonLogementState> emit) =>
      emit(state.copyWith(estProprietaire: event.valeur));

  void _onSuperficieChange(final MonLogementSuperficieChange event, final Emitter<MonLogementState> emit) =>
      emit(state.copyWith(superficie: event.valeur));

  void _onPlusDe15AnsChange(final MonLogementPlusDe15AnsChange event, final Emitter<MonLogementState> emit) {
    emit(state.copyWith(plusDe15Ans: event.valeur));
  }

  void _onDpeChange(final MonLogementDpeChange event, final Emitter<MonLogementState> emit) {
    emit(state.copyWith(dpe: event.valeur));
  }

  Future<void> _onMiseAJourDemandee(final MonLogementMiseAJourDemandee event, final Emitter<MonLogementState> emit) async {
